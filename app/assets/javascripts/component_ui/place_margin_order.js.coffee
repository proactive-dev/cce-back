@PlaceMarginOrderUI = flight.component ->
  @attributes
    formSel: 'form'
  ...
  @getPrice = ->
    if gon.price_config.price_type == 'fixed'
      BigNumber(gon.price_config.price)
    else
      BigNumber(gon.ticker.last)
  ...
  @allIn = (event)->
    switch @panelType()
      when 'ask'
        @trigger 'place_order::input::price', {price: @getPrice()}
  ...
  @refreshBalance = (event, data) ->
    ...
    @select('currentBalanceSel').text(formatter.fix(type, available_margin_balance))

    @trigger 'place_order::balance::change', balance: BigNumber(available_margin_balance)
    @trigger "place_order::max::#{@usedInput}", max: BigNumber(available_margin_balance)
